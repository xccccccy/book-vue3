<template>
  <div
    class="flex flex-col dark-transition rounded-md shadow-2xl dark:shadow-none bg-white dark:bg-slate-800 backdrop-filter bg-opacity-50 dark:bg-opacity-50 p-2 config w-1/3 xl:w-1/4"
        ...
        <div class="mx-5 inline-block w-1/2">
          ...
          <div v-if="one_config.type == 'select'">
            <el-select v-model="one_config.value" placeholder="Select" size="default" class="w-32">
              <el-option v-for="item in one_config.options" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </div>
