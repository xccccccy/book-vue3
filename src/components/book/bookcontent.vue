<template>
  <div class="app w-full" :style="{ background: setting.currentbjcolor }">
      ...
      <div class="tool tool-left-bottom flex justify-evenly sm:block w-full sm:w-max">
        ...
        <div id="search" title="搜索" @mouseover="search_span_show = true" @mouseout="search_span_show = false">
          <a href="javascript:void(0);">
            ...
            <input v-show="search_span_show" v-model="search_string" placeholder="搜索" @keyup.enter="searchbook()" />
          </a>
        </div>
          ...
          <transition name="bounce">
            <div class="child-setting fixed w-full right-0 top-40 sm:absolute sm:w-max sm:top-0 sm:right-16"
              ...
              <div>
                ...
                ]" :style="{ fontSize: fontsize + 'rem' }" @click="setfontsize(fontsize)">{{ fontsize }}</span>
                ...
                <span class="fontsize-show" style="margin: 0 .3rem"
                  :style="{ fontSize: setting.currentFontsize + 'rem' }">{{ setting.currentFontsize }}</span>
                <span class="fontsize-select-jj" @click="setfontsize(+0.1)"
                  onselectstart="() => {return false}">Aa+</span>
              </div>
              <div id="skin-select">
      ...
      <div class="catalogue-all relative max-w-full">
        <div class="catalogue-container">
          ...
          <div class="catalogue-list">
            <div v-for="(catalogue, index) in catalogue_list" :key="index + 1" @click="jumpcontent(index + 1)"
              :id="'catalogue_' + (index + 1)">{{ catalogue }}</div>
          </div>
        </div>
    ...
    <div v-show="mask_show" class="back-mask bg-slate-500 bg-opacity-20 sm:bg-transparent" @click="mask_click"></div>
  </div>
</template>
